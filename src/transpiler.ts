...
/**
 * Spark Script transpiler (final)
 * - Supports custom interface types (::MyType)
 * - Removes interface blocks before parsing (fix reserved keyword error)
 * - Validates assignments, function param types, call-once semantics
 * - Emits plain JS (collapses double semicolons)
 */
...
/* Main transpiler */
export function transpileSpark(sourceCode: string, filePath = "<input>.sp"): string {
  ...
  for (let idx = 0; idx < origLines.length; idx++) {
        ...
        if (declaredType && interfaces.has(String(declaredType)) && expr.trim().startsWith("{")) {
          ...
          if (!objectLiteralHasKeys(expr, Object.keys(iface.fields))) {
            throw new CompileError(filePath, lineNo, 1, 
              `Interface error: object assigned to '${name}' does not satisfy interface '${declaredType}'. Required fields: ${Object.keys(iface.fields).join(", ")}`);
          }
        }
      }
    ...
    if (assignMatch) {
      ...
      if (existingVar.declaredType && interfaces.has(String(existingVar.declaredType)) && expr.trim().startsWith("{")) {
        ...
        if (!objectLiteralHasKeys(expr, Object.keys(iface.fields))) {
          throw new CompileError(filePath, lineNo, 1, 
            `Interface error: assignment to '${name}' does not satisfy interface '${existingVar.declaredType}'. Required fields: ${Object.keys(iface.fields).join(", ")}`);
        }
      }
  ...
  while ((tom = typedObjRx.exec(sourceCode)) !== null) {
    ...
    if (interfaces.has(ifaceName)) {
      ...
      if (!objectLiteralHasKeys(objLiteral, Object.keys(iface.fields))) {
        ...
        throw new CompileError(filePath, lineNo, 1, 
          `Interface error: object assigned to '${varname}' does not satisfy interface '${ifaceName}'. Required fields: ${Object.keys(iface.fields).join(", ")}`);
      }
    }
  }
